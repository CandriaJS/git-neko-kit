...
export async function get_local_git_repo_info(
	...
	try {
		...
		if (
			!(await exists(path.join(local_path))) ||
			!(await exists(path.join(local_path, ".git")))
		)
			return null;
...
export async function get_local_git_repos_list(
		...
		try {
			...
			if (loop && currentDepth < maxDepth) {
				...
				const subResults = await Promise.all(
					dirItems
						.filter((item) => item.isDirectory() && !dir.includes(item.name))
						.map(async (item) => {
	...
	try {
		...
		if (!(await is_installed_git()))
			throw new Error(GitClientNotInstalledMsg);
 ...
 * ```
 */
export async function get_local_repo_default_branch(
	local_path: string,
): Promise<string> {
	if (!local_path) throw new Error(MissingLocalRepoPathMsg);
	try {
 ...
 * ```
 */
export async function get_remote_repo_default_branch(
	remote_url: string,
): Promise<string> {
	...
	try {
		...
		const defaultBranchMatch = remoteInfo.match(
			/^ref: refs\/heads\/([^\t\n]+)/m,
		);

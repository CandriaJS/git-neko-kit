...
export class Pull_Request extends GitHubClient {
		...
		try {
			...
			if (res.data) {
				...
				const PrData: PullRequestInfoResponseType = {
					...
					state:
						res.data.merged_at && !isEmpty(res.data.merged_at)
							? "merged"
							: res.data.state,
					...
					base: {
						...
						user: {
							...
							name: isEmpty(res.data.base.user.name)
								? null
								: res.data.base.user.name,
					...
					head: {
						...
						user: {
							...
							name: isEmpty(res.data.head.user.name)
								? null
								: res.data.head.user.name,
					...
					assignee: !isEmpty(res.data.assignee)
						? {
								...
								name: isEmpty(res.data.assignee.name)
									? null
									: res.data.assignee.name,
								html_url: res.data.assignee.html_url,
								avatar_url: res.data.assignee.avatar_url,
							}
						: null,
					assignees: !isEmpty(res.data.assignees)
						? res.data.assignees.map(
								(assignee: Record<string, any>): PrUser => ({
									id: assignee.id,
			...
			throw new Error(
				`[GitHub] 获取拉取请求信息失败： ${(error as Error).message}`,
			);
		}
	}
	...
	public async get_get_pull_request_list(
		...
		try {
			...
			if (queryOptions.direction && !queryOptions.sort)
			...
			if (res.data) {
				const PrData: PullRequestListResponseType = await Promise.all(
					res.data.map(
						async (
							pr: Record<string, any>,
						): Promise<PullRequestInfoResponseType> => ({
							id: pr.id,
							...
							base: {
								...
								user: {
									...
									name: isEmpty(pr.base.user.name)
										? pr.base.user.login
										: pr.base.user.name,
							...
							milestone: !isEmpty(pr.milestone)
								? {
										...
										created_at: this.format
											? await format_date(pr.milestone.created_at)
											: pr.milestone.created_at,
										updated_at: this.format
											? await format_date(pr.milestone.updated_at)
											: pr.milestone.updated_at,
							...
							labels: !isEmpty(pr.labels)
								? pr.labels.map(
										(label: Record<string, any>): IssueLabelType => ({
											id: label.id,
											name: label.name,
											color: label.color,
										}),
			...
			throw new Error(
				`[GitHub] 获取拉取请求列表失败： ${(error as Error).message}`,
			);
		}
	}
	...
	public async create_pull_request(
		...
		try {
			...
			const res = await this.post(`/repos/${owner}/${repo}/pulls`, body);
	...
	public async get_pull_request_diff(
		...
		try {
			...
			const res = await this.get(
				`/${owner}/${repo}/pull/${pr_number}.diff`,
				null,
				{
					Accept: "text/plain",
				},
			...
			throw new Error(
				`[GitHub] 获取拉取请求diff失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async get_pull_request_files_list(
		...
		try {
			...
			if (res.data) {
				const PrData: GetPullRequestFilesListResponseType = res.data.map(
					(file: Record<string, any>): PullRequestFilesListType => ({
						sha: file.sha,
			...
			throw new Error(
				`[GitHub] 获取拉取请求文件列表失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async get_pull_request_comment_info(
		...
		if (!options.comment_id)
			throw new Error(MissingPullRequestCommentNumberMsg);
		try {
			this.setRequestConfig({
				token: this.userToken,
			});
			...
			throw new Error(
				`[GitHub] 获取拉取请求评论评论信息失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async get_pull_request_comments_list(
		...
		try {
			...
			if (res.data) {
				const PrData: GetPullRequestCommentsListResponseType =
					await Promise.all(
						res.data.map(
							async (
								comment: Record<string, any>,
							): Promise<GetPullRequestCommentInfoResponseType> => {
								return {
									id: comment.id,
									body: comment.body,
									user: {
									...
									created_at: this.format
										? await format_date(comment.created_at)
										: comment.created_at,
									updated_at: this.format
										? await format_date(comment.updated_at)
										: comment.updated_at,
								};
							},
						),
			...
			throw new Error(
				`[GitHub] 获取拉取请求评论列表失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async create_pull_request_comment(
		...
		try {
			...
			throw new Error(
				`[GitHub] 创建拉取请求评论失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async update_pull_request_comment(
		...
		try {
			...
			throw new Error(
				`[GitHub] 更新拉取请求评论失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async delete_pull_request_comment(
		...
		try {
			const res = await this.delete(
				`/repos/${options.owner}/${options.repo}/pulls/comments/${options.comment_id}`,
			);
			switch (res.statusCode) {
				case 403:
					throw new Error(PermissionDeniedMsg);
			...
			throw new Error(
				`[GitHub] 删除拉取请求评论失败: ${(error as Error).message}`,
			);
		}
	}
}

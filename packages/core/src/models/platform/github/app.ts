...
export class App extends GitHubClient {
	 ...
	 * ```
	 */
	public async get_app_info_by_auth(): Promise<
		ApiResponseType<AppInfoResponseType>
	> {
	...
	public async get_app_installation_by_repo(
		...
		try {
			...
			if (res.data) {
				...
				const AppData: GetAppInfoByRepoResponseType = {
					...
					account: {
						...
						email: isEmpty(res.data.account.email)
							? null
			...
			throw new Error(
				`[GitHub] 获取存储库安装应用信息失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async create_access_token_for_app(
		...
		try {
			...
			const body: Record<
				string,
				string | Array<number> | Array<string> | AccessTokenPermissionsType
			> = {};
			...
			const res = await this.post(
				`app/installations/${options.installation_id}/access_tokens`,
				body,
			);
			if (res.statusCode === 201 && res.data) {
				const expiresAt = this.format
					? await format_date(res.data.expires_at)
					: res.data.expires_at;
						...
						res.data.repositories.map(
							async (
								...
								owner: {
									...
									name: isEmpty(res.data.owner.name)
										? null
										: res.data.owner.name,
			...
			throw new Error(
				`[GitHub] 为应用程序创建访问令牌失败: ${(error as Error).message}`,
			);
		}
	}
	 ...
	 */
	public async revoke_access_token_for_app(): Promise<
		ApiResponseType<RevokeAccessTokenResponseType>
	> {
		this.setRequestConfig({
			token: this.jwtToken,
		});
	...
	public async create_install_link(state_id?: string): Promise<string> {
		try {
			if (!this.is_app_client) throw new Error(MissingAppClientMsg);
			const url = new URL(
				`apps/${await this.get_app_name()}/installations/new`,
				this.base_url,
			);
			...
			throw new Error(
				`[GitHub] 生成应用安装链接失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async create_config_install_link(state_id?: string): Promise<string> {
		try {
			...
			throw new Error(
				`[GitHub] 生成应用配置链接失败: ${(error as Error).message}`,
			);
		}
	}

...
} from "@candriajs/git-neko-kit-utils";
...
export class Repo extends GitHubClient {
	...
	public async get_org_repos_list(
		...
		try {
			...
			throw new Error(
				`[GitHub] 获取组织仓库列表失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async get_user_repos_list_by_token(
		...
		try {
			...
			throw new Error(
				`[GitHub] 获取授权用户仓库列表失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async get_user_repos_list(
		...
		try {
			...
			throw new Error(
				`[GitHub] 获取用户仓库列表失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async get_repo_languages_list(
		...
		try {
			...
			if (res.data) {
				...
				const languages: LanguageInfo[] = entries.map(
					([language, bytes]): LanguageInfo => ({
						language,
						color: get_langage_color(language.toLowerCase()),
						percent: Number(
							(((bytes as number) / totalBytes) * 100).toFixed(2),
						),
			...
			throw new Error(
				`[GitHub] 获取仓库语言列表失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async create_org_repo(
		...
		try {
			...
			if (repoOptions.description)
				body.description = repoOptions.description.toString();
			if (repoOptions.homepage) body.homepage = repoOptions.homepage.toString();
			if (repoOptions.visibility)
				body.private = repoOptions.visibility.toString();
	...
	public async get_collaborators_list(
		...
		try {
			...
			const params: Record<string, string> = {};
			if (queryOptions.affiliation)
				params.milestone = queryOptions.affiliation.toString();
			if (queryOptions.permission)
				params.permission = queryOptions.permission.toString();
			if (queryOptions.per_page)
				params.per_page = queryOptions.per_page.toString();
			if (queryOptions.page) params.page = queryOptions.page.toString();
			...
			if (res.data) {
				const RepoData = await Promise.all(
					res.data.map(
						(repo: Record<string, any>): CollaboratorInfoResponseType => ({
							id: repo.id,
							login: repo.login,
							...
							permissions:
								repo.permissions === "triage" || repo.permissions === "maintain"
									? "admin"
									: repo.permissions,
						}),
					),
			...
			throw new Error(
				`[GitHub] 获取仓库协作者列表失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async get_repo_visibility(
		...
		try {
			...
			throw new Error(
				`[GitHub] 获取仓库可见性失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async get_repo_default_branch(
		...
		try {
			const { owner, repo } = options;
			const github_url =
				get_base_url(this.type, { proxyType: ProxyType.Original }) +
				"/" +
				owner +
				"/" +
				repo;
			let default_branch;
			const [error, response] = await tryit(get_remote_repo_default_branch)(
				github_url,
			);
			if (error) {
				default_branch = (await this.get_repo_info({ owner, repo })).data
					.default_branch;
			} else {
			...
			throw new Error(
				`[GitHub] 获取仓库默认分支失败: ${(error as Error).message}`,
			);
		}
	}

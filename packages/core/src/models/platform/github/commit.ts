...
import { format_date, parse_git_url } from "@candriajs/git-neko-kit-utils";
...
export class Commit extends GitHubClient {
	...
	public async get_commit_info(
		...
		try {
			...
			if (!options.sha) {
				...
				const default_branch = await repoInfo.get_repo_default_branch({
					owner,
					repo,
				});
				sha = default_branch;
			...
			if (res.data) {
				const [authorDate, committerDate] = await Promise.all([
					this.format
				...
				const CommitData: CommitInfoResponseType = {
					...
					commit: {
						...
						author: {
							...
							name: isEmpty(res.data.commit.author.name)
								? null
								: res.data.commit.author.name,
							email: isEmpty(res.data.commit.author.email)
								? null
								: res.data.commit.author.email,
						...
						committer: {
							...
							name: isEmpty(res.data.commit.committer.name)
								? null
								: res.data.commit.committer.name,
							email: isEmpty(res.data.commit.committer.email)
								? null
								: res.data.commit.committer.email,
	...
	public async get_commit_list(
		...
		try {
			...
			if (res.data) {
				const CommitData: CommitListResponseType = await Promise.all(
					res.data.map(
						async (
							commit: Record<string, any>,
						): Promise<CommitInfoResponseType> => {
							...
							return {
								...
								commit: {
									...
									author: {
										...
										name: isEmpty(commit.committer.name)
											? null
											: commit.committer.name,
										email: isEmpty(commit.committer.email)
											? null
									...
									committer: {
										...
										type: capitalize(
											String(commit.committer.type).toLowerCase(),
										),
										date: this.format

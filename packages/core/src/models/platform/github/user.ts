...
import {
	...
	ProxyType,
	type UserInfoByIdParamType,
	type UserInfoParamType,
	type UserInfoResponseType,
	type UserNameParamType,
} from "@/types";
...
export class User extends GitHubClient {
	...
	public async get_user_info(
		...
		try {
			...
			throw new Error(
				`[GitHub] 获取用户织信息失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async get_user_info_by_user_id(
		...
		try {
			...
			throw new Error(
				`[GitHub] 通过用户id获取用户信息失败: ${(error as Error).message}`,
			);
		}
	}
	 ...
	 * ```
	 */
	public async get_user_info_by_auth(): Promise<
		ApiResponseType<UserInfoResponseType>
	> {
		if (!this.userToken) {
		...
		try {
			...
			throw new Error(
				`[GitHub] 获取授权用户信息失败: ${(error as Error).message}`,
			);
		}
	}
	...
	public async get_user_info_by_token(): Promise<
		ApiResponseType<UserInfoResponseType>
	> {
	 ...
	 * ```
	 */
	public async get_user_contribution(
		options: UserNameParamType,
		...
		try {
			...
			throw new Error(
				`[GitHub] 获取用户贡献信息失败: ${(error as Error).message}`,
			);
		}
	}

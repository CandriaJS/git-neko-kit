...
export async function get_local_npm_packages_list (
  ...
  const packagePromises = filteredDeps.map(pkg => get_local_npm_package_info(pkg))
  const packageInfos = (await Promise.all(packagePromises))
    .filter((info): info is NpmPackageInfoType => info !== null)
